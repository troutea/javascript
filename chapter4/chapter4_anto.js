...
function alertFunction()
{
 ...
 //Element Creation and Insertion: Describe how to create a new <li> element, set its text
 //content to New Item, and append it to an existing <ul> with the ID list container
 ...
 //5. Form Handling: Write a script to prevent the default submission of a form with the ID form-example
 //and instead console-log the value of a text input field within the form
...
function stopBtnPropogation(e) {
    e.stopBtnPropogation
}
propBtn.addEventListener('click', stopBtnPropogation)

//7 Dynamic Page Styling: How can you toggle a class active on an element when it is
// clicked , ensuring that if the class is present it gets removed and if its not it gets added? 
...
//8. Advanced Event Handling: Write a Javascript code to change the text content of a paragraph
// element to displey the current time when a button is clicked.

let paragraphElement = document.querySelector('p')
paragraphElement.innerText = new Date()


//9. Event Listener Removal: How do you remove an event Listener from an element , and why
// might you need to do this?
document.getElementById('james-is-cool').removeEventListener('click', myFunction)


//10. Manipulating Attributes and Properties: Write JavaScript code to change the src
//attribute of an <img> element and explain the difference between attributes and 
// properties in the context of DOM elements
